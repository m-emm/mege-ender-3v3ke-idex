...
from . import ldc1612, manual_probe, probe, trigger_analog
...
class EddyCalibrationTool:
    ...
    def validate_calibration_data(self, positions):
        last_freq = 40000000.0
        last_pos = last_mad = 0.0
        gcode = self.printer.lookup_object("gcode")
        filtered = []
        mad_hz_total = 0.0
        mad_mm_total = 0.0
        ...
        for pos, (freq_avg, mad_hz, count) in sorted(positions.items()):
            if freq_avg > last_freq:
                gcode.respond_info("Frequency stops decreasing at step %.3f" % (pos))
                break
            ...
            if freq_diff < 2.5 * diff_mad:
                gcode.respond_info(
                    "Frequency too noisy at step %.3f -> %.3f" % (last_pos, pos)
                )
                gcode.respond_info(
    ...
    def post_manual_probe(self, mpresult):
        ...
        if len(filtered) <= 8:
            raise self.printer.command_error("Failed calibration - No usable data")
    ...
    def _save_tap_z_offset(self, gcmd, homing_z):
        ...
        tap_z_offset = csettings.get("tap_z_offset", 0.0)
...
class EddyTapCalibration:
    ...
    def _describe_last_tap(self, last_tap):
        ...
        m2 = "  depress_dist=%.6f depress_slope=%.3f" % (depress_dist, depress_slope)
    ...
    def _try_tap(self, gcmd, tap_threshold, samples=1):
        ...
        gcmd.respond_info(
            "Tap probing with TAP_THRESHOLD=%s SAMPLES=%s"
    ...
    def _save_tap_threshold(self, gcmd, tap_threshold):
        ...
        gcmd.respond_info(
            "%s: tap_threshold: %.3f\n"
            "The SAVE_CONFIG command will update the printer config file\n"
            "with the above and restart the printer." % (self._name, tap_threshold)
        )
    ...
    def cmd_TAP_CALIBRATE(self, gcmd):
        ...
        if tap_test is None:
            ...
            if mc_coeffs is None:
                raise gcmd.error("Must complete PROBE_EDDY_CURRENT_CALIBRATE first")
...
class EddyGatherSamples:
    def __init__(self, printer, sensor_helper):
        ...
        # Trace requests are deliberately a side channel: they must never
        # add a second item to the normal PROBE result queue.  This keeps
        # TRACE=1 observational and preserves the regular tap API exactly.
    ...
    def _check_sensor_messages(self):
        ...
        while self._sensor_messages and self._trace_requests:
            cb, start_time, end_time, args = self._trace_requests[0]
            if self._sensor_messages[-1]["data"][-1][0] < end_time:
                break
            measures = self._pull_measurements(start_time, end_time)
            try:
                ...
                res = exc
            self._trace_results.append(res)
            self._trace_requests.pop(0)

    def add_probe_request(self, cb, start_time, end_time, *args):
        self._probe_requests.append((cb, start_time, end_time, args))
        self._check_sensor_messages()

    def add_trace_request(self, cb, start_time, end_time, *args):
        self._trace_requests.append((cb, start_time, end_time, args))
        self._check_sensor_messages()
    ...
    def _await_sensor_messages(self):
        ...
        while self._probe_requests or self._trace_requests:
            ...
            if mcu.is_fileoutput():
                ...
                dummy_pr = manual_probe.create_probe_result(
                    (
                        0.0,
                        0.0,
                        0.0,
                ...
                if self._probe_requests:
                    ...
                    self._trace_results.append(None)
                    self._trace_requests.pop(0)
                continue
            reactor.pause(systime + 0.010)

    def pull_trace_results(self):
        self._await_sensor_messages()
        results = list(self._trace_results)
        del self._trace_results[:]
...
def probe_results_from_avg(measures, toolhead_pos, calibration, offsets):
    ...
    return manual_probe.create_probe_result(
        toolhead_pos, (offsets[0], offsets[1], sensor_z)
    )
...
class EddyDescend:
    def __init__(
        ...
        if (
            config.get("z_offset", None, note_valid=False) is not None
            and config.get("descend_z", None, note_valid=False) is None
        ):
            config.deprecate("z_offset")
            self._descend_z = config.getfloat("z_offset", above=0.0)
        else:
            self._descend_z = config.getfloat("descend_z", above=0.0)
    ...
    def _prep_trigger_analog(self):
        ...
        sos_filter.set_offset_scale(0, 1.0)
    ...
    def run_probe(self, gcmd):
        ...
        self._gather.add_probe_request(
            probe_results_from_avg,
            start_time,
            end_time,
            toolhead_pos,
            self._calibration,
...
class EddyTap:
    # The existing fit already rejects contacts outside this physical
    # post-trigger interval.  Keep these values in one place so candidate
    # selection cannot optimise over the whole retract and then reject the
    # result only after the fact.
    TAP_CONTACT_MIN_DIST = 0.030
    TAP_CONTACT_MAX_DIST = 0.250
    TAP_RELEASE_GUARD_MM = 0.010
    ...
    def __init__(
        self, config, sensor_helper, param_helper, trigger_analog, calibration
    ):
        self._printer = config.get_printer()
        self._sensor_helper = sensor_helper
        self._param_helper = param_helper
        self._trigger_analog = trigger_analog
        # TRACE converts each captured native frequency back through the same
        # calibration used by the tap detector.  Keep the calibration object
        # on the tap session itself; the generic ProbeSessionHelper wrapper
        # does not provide it and leaving this unset silently drops every
        # converted-height sample from the diagnostic plot.
        self._calibration = calibration
        self._z_min_position = probe.lookup_minimum_z(config)
        self._gather = None
        self._filter_design = None
        self._tap_z_offset = config.getfloat("tap_z_offset", 0.0)
        self._tap_threshold = config.getfloat("tap_threshold", 0.0, above=0.0)
        self._least_squares_cache = {}
        self._current_tap_threshold = 0.0
        self._setup_tap()
        self._last_tap = None
        self._last_tap_diagnostics = None
        self._trace_enabled = False
        self._last_tap_trace = None
    ...
    def _prep_trigger_analog_tap(self, gcmd):
        tap_threshold = gcmd.get_float("TAP_THRESHOLD", self._tap_threshold, above=0.0)
    ...
    def _validate_samples_time(self, measures, start_time, end_time):
        ...
        if tmax > cycle_time * 1.25:
            raise cmderr(
                "Eddy: Gaps in the data: %.3f > %.3f" % (tmax, cycle_time * 1.25)
            )
        if tmin < cycle_time * 0.75:
            raise cmderr(
                "Eddy: CLKIN frequency too low: %.3f < %.3f" % (tmin, cycle_time * 0.75)
            )
    ...
    def _build_ls_matrix(self, samples, est_z_contact):
        ...
        eqs = [[0.0] * 4 for i in range(len_samples)]
    ...
    def _build_sums(self, samples, num_le):
        sum_le_z = sum_le_z2 = sum_le_freq = sum_le_freq_z = 0.0
        for z, freq in samples[:num_le]:
            sum_le_z += z
            sum_le_z2 += z**2
            sum_le_freq += freq
            sum_le_freq_z += freq * z
        sum_gt_z = sum_gt_z2 = sum_gt_z3 = sum_gt_z4 = 0.0
        sum_gt_freq = sum_gt_freq_z = sum_gt_freq_z2 = 0.0
        for z, freq in samples[num_le:]:
            sum_gt_z += z
            sum_gt_z2 += z**2
            sum_gt_z3 += z**3
    ...
    def _build_ls_matrix_opt(self, samples, est_z_contact):
        ...
        eqst_eqs = [[0.0] * 4 for i in range(4)]
    ...
    def _calc_least_squares(self, samples, est_z_contact):
        ...
        if coeffs is not None and coeffs[3][0] < 0.0:
            ...
            if coeffs is not None:
                coeffs = coeffs + [[0.0]]
        if coeffs is None:
            return sys.float_info.max, [[0.0]] * 4
    ...
    def _find_least_squares(self, data):
        ...
        base_z = 0.5 * (data[0][1][2] + data[-1][1][2])
        base_freq = 0.5 * (data[0][0] + data[-1][0])
    ...
    def _fit_line(samples):
        """Return intercept, slope, and squared residual for (z, height)."""
        count = len(samples)
        if count < 2:
            return None
        mean_z = sum(sample[0] for sample in samples) / count
        mean_height = sum(sample[1] for sample in samples) / count
        variance_z = sum((z - mean_z) ** 2 for z, _height in samples)
        if variance_z <= 0.0:
            return None
        covariance = sum((z - mean_z) * (height - mean_height) for z, height in samples)
        slope = covariance / variance_z
        intercept = mean_height - slope * mean_z
        residual = sum((height - (intercept + slope * z)) ** 2 for z, height in samples)
        return intercept, slope, residual

    def _find_release_fit(self, data, candidate_min_z, candidate_max_z):
        """Select the best physically valid release intersection.

        The calibration converts native Eddy frequency to probe height, so a
        free nozzle should have a slope close to +1 against commanded Z while
        the loaded leg remains comparatively flat.  Each plausible split is
        evaluated independently; no unconstrained optimum can pre-empt a
        physically valid release.
        """
        height_samples = []
        invalid_height_samples = 0
        for frequency, position in data:
            height = self._calibration.freq_to_height(frequency)
            if not math.isfinite(height) or abs(height) >= OUT_OF_RANGE:
                invalid_height_samples += 1
                continue
            height_samples.append((float(position[2]), float(height)))
        candidates = sorted(
            {
                z
                for z, _height in height_samples
                if candidate_min_z <= z <= candidate_max_z
            }
        )
        diagnostics = {
            "candidate_count": len(candidates),
            "candidate_min_z": candidate_min_z,
            "candidate_max_z": candidate_max_z,
        ...
        for split_z in candidates:
            contact = [
                (z, height)
                for z, height in height_samples
                if z <= split_z - self.TAP_RELEASE_GUARD_MM
            ]
            free = [
                (z, height)
                for z, height in height_samples
            ...
            if (
                len(contact) < self.TAP_MIN_LEG_SAMPLES
                or len(free) < self.TAP_MIN_LEG_SAMPLES
            ):
                reject("insufficient_samples")
                continue
            contact_span = contact[-1][0] - contact[0][0]
            free_span = free[-1][0] - free[0][0]
            if (
                contact_span < self.TAP_MIN_CONTACT_SPAN_MM
                or free_span < self.TAP_MIN_FREE_SPAN_MM
            ):
                reject("insufficient_span")
                continue
            contact_fit = self._fit_line(contact)
            free_fit = self._fit_line(free)
            if contact_fit is None or free_fit is None:
                reject("singular_line_fit")
                continue
            contact_intercept, contact_slope, contact_residual = contact_fit
            free_intercept, free_slope, free_residual = free_fit
            slope_delta = free_slope - contact_slope
            if not (
                self.TAP_FREE_HEIGHT_SLOPE_MIN
                <= free_slope
                <= self.TAP_FREE_HEIGHT_SLOPE_MAX
            ):
                reject("free_slope")
                continue
            if slope_delta < self.TAP_MIN_HEIGHT_SLOPE_DELTA:
                reject("slope_delta")
                continue
            intersection_z = (contact_intercept - free_intercept) / (
                free_slope - contact_slope
            )
            intersection_error = abs(intersection_z - split_z)
            intersection_limit = max(2.0 * self.TAP_RELEASE_GUARD_MM, 0.015)
            if (
                intersection_error > intersection_limit
                or intersection_z < candidate_min_z
                or intersection_z > candidate_max_z
            ):
                reject("intersection")
                continue
            residual = contact_residual + free_residual
            sample_count = len(contact) + len(free)
            score = (
                residual / sample_count
                + 0.001 * (free_slope - 1.0) ** 2
                + 0.001 * contact_slope**2
                + 0.001 * (intersection_error / intersection_limit) ** 2
            )
            fit = {
                "z_contact": intersection_z,
                "split_z": split_z,
                "intersection_error_mm": intersection_error,
                "contact_height_intercept": contact_intercept,
                "contact_height_slope": contact_slope,
                "free_height_intercept": free_intercept,
                "free_height_slope": free_slope,
                "height_slope_delta": slope_delta,
                "contact_residual": contact_residual,
                "free_residual": free_residual,
                "residual": residual,
                "score": score,
                "contact_sample_count": len(contact),
                "free_sample_count": len(free),
                "contact_span_mm": contact_span,
                "free_span_mm": free_span,
            }
            if best is None or fit["score"] < best["score"]:
                best = fit
        diagnostics["valid_candidate_count"] = len(candidates) - sum(
            diagnostics["rejections"].values()
        )
    ...
    def _record_tap_trace(
        ...
        """Correlate one complete tap trace without affecting tap analysis."""
        rows = []
        trace_errors = []
        for samp_time, sensor_freq, stream_height in measures:
            try:
                commanded = self._lookup_toolhead_pos(samp_time)
                commanded_x, commanded_y, commanded_z = commanded[:3]
            except Exception as exc:
                commanded_x = commanded_y = commanded_z = None
                trace_errors.append(str(exc))
            try:
                converted_height = float(self._calibration.freq_to_height(sensor_freq))
            except Exception as exc:
                converted_height = None
                trace_errors.append(str(exc))
            if samp_time < trigger_time:
                phase = "descent"
            elif samp_time < retract_start_time:
                phase = "trigger"
            else:
                phase = "retract"
            rows.append(
                {
                    "mcu_time": float(samp_time),
                    "frequency_hz": float(sensor_freq),
                    "stream_height_mm": float(stream_height),
                    "converted_height_mm": converted_height,
        ...
        self._last_tap_trace = {
            "status": status,
            "trace_start": float(trace_start),
            "trace_end": float(trace_end),
            "trigger_time": float(trigger_time),
            "retract_start_time": float(retract_start_time),
            "retract_end_time": float(retract_end_time),
            "rows": rows,
            "trace_errors": trace_errors,
        }

    def _record_no_trigger_trace(self, measures, trace_start, trace_end):
        # There is no analog trigger timestamp in this case.  Treat the end
        # of the attempted descent as the event boundary while retaining the
        # complete available sensor record for diagnosing the miss.
        self._record_tap_trace(
            measures,
            trace_start,
            trace_end,
            trace_end,
    ...
    def get_last_tap_trace(self):
        return self._last_tap_trace

    def flush_tap_trace(self):
        if self._gather is not None:
            self._gather.pull_trace_results()
    ...
    def _analyze_pullback(
        ...
        self._last_tap_diagnostics = None
        reactor = self._printer.get_reactor()
        # Fit only the established retract window.  The trace request spans
        # the descent as well, so diagnostics show the full motion while the
        # tap detector retains its original fitting input exactly.
        fit_measures = [
            measure
            for measure in measures
            if fit_start_time <= measure[0] <= fit_end_time
        ]
        ...
        self._validate_samples_time(fit_measures, fit_start_time, fit_end_time)
        # Correlate measurements to toolhead position at time of measurement
        data = [
            (sensor_freq, self._lookup_toolhead_pos(samp_time))
            for samp_time, sensor_freq, sensor_z in fit_measures
        ]
        reactor.pause(0.0)
        min_z = data[0][1][2]
        max_z = data[-1][1][2]
        # Populate aggregate diagnostics before any early validation failure.
        # A short/invalid pullback is still useful evidence: the host helper
        # can report the actual sampled Z/frequency range without retaining
        # the large raw trace that used to obscure the operator-facing error.
        self._last_tap_diagnostics = {
            "sample_count": len(measures),
            "min_frequency_hz": float(min(m[1] for m in measures)),
            "max_frequency_hz": float(max(m[1] for m in measures)),
            "min_toolhead_z": float(min_z),
            "max_toolhead_z": float(max_z),
            # The first correlated sample is the physical nozzle halt.  Keep
            # it in the compact diagnostics so the host can distinguish a
            # rejected trigger exactly at a discovery-band boundary from a
            # rejected trigger in the interior of a band.
            "trigger_z": float(min_z),
            "pullback_lift": float(max_z - min_z),
            "tap_threshold": float(self._current_tap_threshold),
        }
        if max_z - min_z < 0.350:
            self._error_detect(
                "insufficient lift (%.6f vs %.6f)" % (max_z - min_z, 0.350)
            )
        candidate_min_z = min_z + self.TAP_CONTACT_MIN_DIST
        candidate_max_z = min_z + self.TAP_CONTACT_MAX_DIST
        release_fit, release_diagnostics = self._find_release_fit(
            data, candidate_min_z, candidate_max_z
        )
        self._last_tap_diagnostics = {
            "tap_threshold": float(self._current_tap_threshold),
            "sample_count": len(measures),
            ...
            "max_toolhead_z": float(max_z),
            "trigger_z": float(min_z),
            "fit_selection": "exhaustive_height_intersection",
            "release_fit": release_fit,
            "release_candidates": release_diagnostics,
            "fit_status": "pending",
        }
        reactor.pause(0.0)
        if release_fit is None:
            failure = (
                "no valid height-space release fit "
                "(candidates=%d, rejected=%s)"
                % (
                    release_diagnostics["candidate_count"],
                    release_diagnostics["rejections"],
                )
            )
            self._last_tap_diagnostics["fit_status"] = "rejected"
            self._last_tap_diagnostics["fit_failure"] = failure
            self._last_tap = ("fail", 0.0, (min_z, 0.0, 0.0, 0.0, 0.0))
            self._error_detect(failure)
        z_contact = release_fit["z_contact"]
        split_z = release_fit["split_z"]
        raw_contact = [
            (position[2], frequency)
            for frequency, position in data
            if position[2] <= split_z - self.TAP_RELEASE_GUARD_MM
        ]
        raw_free = [
            (position[2], frequency)
            for frequency, position in data
        ...
        raw_contact_fit = self._fit_line(raw_contact)
        raw_free_fit = self._fit_line(raw_free)
        if raw_contact_fit is None or raw_free_fit is None:
            failure = "valid height fit but unable to summarize native slopes"
            self._last_tap_diagnostics["fit_status"] = "rejected"
            self._last_tap_diagnostics["fit_failure"] = failure
            self._last_tap = (
                "fail",
                z_contact - min_z,
                (z_contact, 0.0, 0.0, 0.0, 0.0),
            )
            self._error_detect(failure)
        raw_contact_intercept, raw_contact_slope, _raw_contact_residual = (
            raw_contact_fit
        )
        raw_free_intercept, raw_free_slope, _raw_free_residual = raw_free_fit
        raw_coeffs = (
            z_contact,
            raw_contact_intercept + raw_contact_slope * z_contact,
            raw_contact_slope,
            raw_free_slope,
            0.0,
        )
        self._last_tap_diagnostics["native_frequency_fit"] = {
            "frequency_contact_hz": raw_coeffs[1],
            "contact_slope": raw_contact_slope,
            "free_slope": raw_free_slope,
            "slope_delta": raw_contact_slope - raw_free_slope,
        }
        self._last_tap_diagnostics["fit_status"] = "accepted"
        self._last_tap = ("success", z_contact - min_z, raw_coeffs)
        if self._last_tap_trace is not None:
            self._last_tap_trace["status"] = "success"
        ...
        # Keep the nozzle's physical position in both result coordinate sets.
        # In METHOD=tap the nozzle, rather than the Eddy coil, makes contact
        # with the bed. The Eddy signal only detects the deformation. Applying
        # the coil offset here would invent a different contact point and
        # corrupt the datum used by EDDY_TAP_MEASURE and mesh tapping.
    ...
    def get_last_tap_diagnostics(self):
        return self._last_tap_diagnostics
    ...
    def run_probe(self, gcmd):
        ...
        pos = toolhead.get_position()
        start_pos = list(pos)
        target_z = gcmd.get_float("TAP_TARGET_Z", self._z_min_position)
        if target_z < self._z_min_position:
            raise self._printer.command_error(
                "TAP_TARGET_Z %.6f is below the Z minimum %.6f"
                % (target_z, self._z_min_position)
            )
        if target_z >= start_pos[2]:
            raise self._printer.command_error(
                "TAP_TARGET_Z %.6f is not below the current Z %.6f"
                % (target_z, start_pos[2])
            )
        pos[2] = target_z
        params = self._param_helper.get_probe_params(gcmd)
        ...
        try:
            ...
            if self._trace_enabled:
                trace_end = toolhead.get_last_move_time() + 0.050
                self._gather.add_trace_request(
                    self._record_no_trigger_trace,
                    trace_start,
                    trace_end,
                    trace_start,
                    trace_end,
        ...
        # Extract retract samples.  The old fixed 150 ms window can end while
        # a deliberately short discovery move is still retracting (this
        # printer's Z motion does not reach the requested lift immediately),
        # producing a misleading "insufficient lift" rejection.  Keep the
        # original minimum window, but extend it to cover the commanded lift
        # plus settling time.  This records the deformation fit, not a raw
        # frequency trace.
        start_time = retract_start_time - 0.010
        retract_window = max(0.150, lift_dist / max(lift_speed, 1.0e-6) + 0.100)
        end_time = retract_start_time + retract_window
        if self._trace_enabled:
            # The single normal probe request carries the complete trace and
            # the original fitting interval, so TRACE adds no second probe
            # result and cannot perturb the caller's result handling.
            trace_start_time = trace_start
        else:
            trace_start_time = start_time
        self._gather.add_probe_request(
            self._analyze_pullback,
            trace_start_time if self._trace_enabled else start_time,
            end_time,
            start_time,
            end_time,
            trace_start_time,
            end_time,
            self._trigger_analog.get_last_trigger_time(),
            retract_start_time,
            end_time,
        )
...
class EddyScanningProbe:
    ...
    def _analyze_scan(self, measures, pos_time):
        ...
        return probe_results_from_avg(
            measures, toolhead_pos, self._calibration, self._offsets
        )
    ...
    def _rapid_lookahead_cb(self, printtime):
        ...
        self._gather.add_probe_request(
            self._analyze_scan, start_time, end_time, printtime
        )
    ...
    def run_probe(self, gcmd):
        ...
        end_time = start_time + self._sample_time
        self._gather.add_probe_request(
            self._analyze_scan, start_time, end_time, start_time
        )
...
class EddyProbeOffsets:
    ...
    def get_offsets(self, gcmd=None):
        return self.x_offset, self.y_offset, 0.0
...
class EddyParameterHelper:
    ...
    def get_probe_params(self, gcmd=None):
        ...
        lift_speed = gcmd.get_float("LIFT_SPEED", 5.0, above=0.0)
        samples = gcmd.get_int("SAMPLES", 1, minval=1)
        samp_retract_dist = 0.0
        samp_tolerance = gcmd.get_float("SAMPLES_TOLERANCE", 0.100, minval=0.0)
...
class PrinterEddyProbe:
    def __init__(self, config):
        ...
        self.eddy_tap = EddyTap(
            config, self.sensor_helper, self.param_helper, trig_analog, self.calibration
        )
        EddyTapCalibration(config, self.calibration, self.eddy_tap)
        # Probing via "scan" and "rapid_scan" requests
        self.eddy_scan = EddyScanningProbe(
            config, self.sensor_helper, self.calibration, self.probe_offsets
        )
        # Register with main probe interface
        self.cmd_helper = probe.ProbeCommandHelper(config, self, can_set_z_offset=False)
    ...
    def get_offsets(self, gcmd=None):
        if gcmd is not None and gcmd.get("METHOD", "").lower() == "tap":
            # A tap is a nozzle-contact measurement. Returning zero offsets
            # tells the generic probe machinery that the coordinate already
            # describes the nozzle contact point. Coil offsets apply only to
            # non-contact descend and scan measurements.
            return (0.0, 0.0, 0.0)
        return self.probe_offsets.get_offsets(gcmd)

    def get_last_tap_diagnostics(self):
        return self.eddy_tap.get_last_tap_diagnostics()
