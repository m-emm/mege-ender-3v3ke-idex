...
class ProbeSessionHelper:
    ...
    def _probe(self, gcmd):
        ...
        epos = results[0]
        # Report results for ordinary probing.  Tap callers provide their own
        # aggregate result and intentionally suppress this per-attempt line.
        if (gcmd.get('METHOD', '').lower() != 'tap'
                and gcmd.get('REPORT', '1') != '0'):
            gcode = self.printer.lookup_object('gcode')
