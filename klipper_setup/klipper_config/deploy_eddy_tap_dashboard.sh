...
check_remote() {
  ssh "${REMOTE_HOST}" "CHECK_LOCAL_ASSETS_SHA='${local_assets_sha}' REMOTE_EDDY_DIR='${REMOTE_EDDY_DIR}' python3 -" <<'PY'
import hashlib
import os
from pathlib import Path
...
ssh "${REMOTE_HOST}" \
  "REMOTE_TMP='${remote_tmp}' REMOTE_EDDY_DIR='${REMOTE_EDDY_DIR}' bash -s" <<'REMOTE_SCRIPT'
set -euo pipefail
