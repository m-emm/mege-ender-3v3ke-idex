...
Print startup loads the source-controlled `default` mesh after homing and does
not run a hot Tap or Eddy scan. The measured points live in `calib.yaml`, are
rendered into `[bed_mesh default]` in the generated printer config, and remain
active across restarts. A mesh update is an explicit calibration and download
workflow, not an implicit print-start action.
...
Klipper's native Tap mesh command makes the new `default` mesh active and
stages its measured profile in `configfile.save_config_pending_items`. The
workflow must download the active profile points, update `tap_mesh.saved_mesh`
in `calib.yaml`, regenerate `printer.cfg`, and deploy it. Do not use
`SAVE_CONFIG`; the checked-in generated profile is the durable source of truth.
...
`BED_MESH_PROFILE SAVE` or `SAVE_CONFIG`; startup `BED_MESH_PROFILE LOAD=default`
is the normal activation path. A failed post-mesh verification leaves `default`
active for supervised inspection but
...
- leave heaters off;
- leave `default` active for the current session and update its checked-in
  points only after downloading and reviewing the measured mesh;
...
Tap defines both the reference contact and the current runtime mesh shape.
Vision-derived relative T0/T1 geometry then transfers the T0 datum to T1.
None of these layers should modify the manual-offset UI.
...
3. Create the native T0 Tap mesh under the recorded conditions.
...
- the verification mesh originates from a deliberate scan, is reviewed and
  downloaded into `tap_mesh.saved_mesh`, and matches the generated
  `[bed_mesh default]` profile;
...
- [ ] Run a deliberate full scan with `(150,150)` as zero reference, review the
      active `default` profile, download its points, and regenerate the
      source-controlled mesh before deployment.
