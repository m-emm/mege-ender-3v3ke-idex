...
6. **Generate the mesh.** Scan the full configured domain with `(150,150)` as
   its zero reference, retain the result as the runtime-only `saved_mesh`
   profile for supervised inspection, and keep it active in Klipper memory.
...
tests default **nozzle** heights `0.5,1,2,3` mm. It first travels safely in XY,
descends once to a small clearance below the first height, then approaches each
measurement height upwards. For each height it reports the
...
at `(207.391,168.997)` for the comparison probe. Its default `EDDY_MODE=probe`
reports one regular probe beside the Tap median. `EDDY_MODE=scan` instead takes
stationary `METHOD=scan` samples at `tap_median + SCAN_HEIGHT`; it requires T0
and a cleared mesh, and reports requested target, Tap XY, scan XY, coil nozzle
pose, statistics, and the signed scan-minus-Tap residual. `XY_SPEED` changes
only safe XY travel; controlled Z moves and the Tap descent are unchanged. If
the derived coil pose is outside current homed limits, Tap statistics remain
valid and the Eddy comparison is skipped with a warning.
...
Klipper's `BED_MESH_CALIBRATE` automatically makes the new mesh active and
stages `[bed_mesh default]` in `configfile.save_config_pending_items`. The
Iteration 1 diagnostic then issues `BED_MESH_PROFILE SAVE=saved_mesh`, solely
to retain an independently loadable runtime profile for manual inspection; it
also remains pending and is never copied into canonical files. The script must
never invoke `SAVE_CONFIG`. Before scanning, pending configuration must still
be empty. After scanning, only `[bed_mesh default]` and `[bed_mesh saved_mesh]`
are permitted pending items; any Eddy, endstop, or unrelated pending item is
an abort condition. Restarting Klipper discards both runtime profiles and their
pending save state.
...
5. verify that the new mesh is active in memory and that only transient
   `[bed_mesh default]` and `[bed_mesh saved_mesh]` pending items exist;
...
when the diagnostic fails, then verifies that an active mesh preserves raw Tap
contact reporting at the largest-correction point.

Klipper tap reports raw low-level kinematic contact, not the logical position
above the bed-mesh transform. Therefore the verification quantity is:
...
Before the 3x3 survey, the clean reference is one same-point Tap/scan
measurement. Its raw Tap must meet the normal per-point tolerance and the mesh
correction at the center must be within +/-0.005 mm. The Tap/scan coordinate
evidence, raw sample, correction, stationary measurement height, and any
failure reason are kept under `reference` in `mesh-verification.json`. A failed
reference gate does not stop the remaining 3x3 survey; the workflow fails only
after all nine grid points have been collected.
...
- each final diagnostic point has one successful Tap and three same-point
  stationary scans with retained coordinate provenance;
...
- leave heaters off;
- leave `saved_mesh` active for inspection until the next restart or print-start
  scan replaces it; it is runtime-only unless an operator explicitly chooses to
  persist it with Klipper's `SAVE_CONFIG` outside this workflow;
