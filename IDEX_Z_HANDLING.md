...
6. **Generate the mesh.** Create one native T0 nozzle-contact Tap mesh over the
   ...
   keep its runtime-only `tap_7x7` profile active in Klipper memory.
...
Klipper's native Tap mesh command automatically makes the new mesh active and
stages `[bed_mesh tap_7x7]` in `configfile.save_config_pending_items`. The
script must never invoke `SAVE_CONFIG` or separately save/reload a mesh profile.
Before meshing, pending configuration must still be empty. Afterwards, only the
transient `[bed_mesh tap_7x7]` section is permitted; any Eddy, endstop, or
unrelated pending item is an abort condition. Restarting Klipper discards the
runtime profile and pending save state.
...
- leave heaters off;
- leave `tap_7x7` active for the current session; it is runtime-only unless an
  operator explicitly chooses to persist it with Klipper's `SAVE_CONFIG` outside
  this workflow;
