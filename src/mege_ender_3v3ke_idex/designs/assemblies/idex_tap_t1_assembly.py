...
def create_idex_tap_t1_assembly(
    *,
    ...
    back_mount_plate = create_box(fixed_tool_head_mount_size[0],
                                  carriage_mount_plate_thickness, 
                                  carriage_size[2])
    
    back_mount_plate = align(
        back_mount_plate,
        carriage,
        Alignment.CENTER, axes=[2]  )
    
    back_mount_plate = align(
        back_mount_plate,
        fixed_tool_head_mount,
        Alignment.CENTER, axes=[0]  )
    ...
    back_factor = 0.5
    new_points = [
        (back_mount_plate_bbox[0][1], back_mount_plate_bbox[0][2]),
          (back_mount_plate_bbox[0][1], back_mount_plate_bbox[1][2]),
          (lower_mount_strips_bbox[1][1], lower_mount_strips_bbox[0][2]),
            ((lower_mount_strips_bbox[0][1] + lower_mount_strips_bbox[1][1] ) * back_factor, lower_mount_strips_bbox[0][2]),
        ((lower_mount_strips_bbox[0][1] + lower_mount_strips_bbox[1][1] ) * back_factor, (back_mount_plate_bbox[0][2] +   back_mount_plate_bbox[1][2]) /2 ),
    ]
