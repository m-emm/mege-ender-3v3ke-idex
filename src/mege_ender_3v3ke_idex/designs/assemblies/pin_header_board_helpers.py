...
def create_sil_pin_line_clamp(
    *,
    ...
    holder_slack,
    base_cutter_vertical_slack,
    lip_size,
    slit_width,
):
    """Create the original slit-and-lip holder for one SIL pin line."""
    ...
    slit_cutter = create_box(
        slit_width,
        pins_size[1] + 4 * dil_pitch,
        base_plate_thickness + 2 * base_cutter_vertical_slack,
    )
    slit_cutter = align(slit_cutter, pins, Alignment.CENTER, axes=[0, 1])
    slit_cutter = align(slit_cutter, base_plate, Alignment.CENTER, axes=[2])
    ...
    retval.add_named_cutter(
        pins.get_cutter_part_by_name("pin_cutters"),
        "pin_cutters",
    )
