...
TOKEN_CENTER_X_MM = (T0_SINGLE_X_MIN_MM + T0_SINGLE_X_MAX_MM) / 2.0
...
def assert_part_inside_temporary_safe_zone(part):
    ...
    if not (
        T0_SINGLE_X_MIN_MM <= min_x <= max_x <= T0_SINGLE_X_MAX_MM
        ...
        raise ValueError(
            ...
            f"safe_x={T0_SINGLE_X_MIN_MM}..{T0_SINGLE_X_MAX_MM}, "
...
def main():
    ...
    process_data["print_area"] = dict(T0_SINGLE_PRINT_AREA)
    ...
    arrange_and_export(
        ...
        prod_gap=4,
        bed_width=T0_SINGLE_BED_WIDTH_MM,
        bed_depth=T0_SINGLE_BED_DEPTH_MM,
        prod_origin=T0_SINGLE_BED_ORIGIN,
    )
