...
- `toolhead_nitehawk_and_x_axis.cfg` - **REFERENCE ONLY** - Monolithic config for documentation
- `x_axis_stepper_endstop_pico_w.cfg` - **EDIT THIS** - X-axis MCU configuration (deployed as include)

**⚠️ IMPORTANT:** Do NOT edit `toolhead_nitehawk_and_x_axis.cfg` directly! This is a reference/monolithic config. Instead:
1. Edit the modular component configs (e.g., `x_axis_stepper_endstop_pico_w.cfg`)
2. Use the install scripts to deploy them
3. The install scripts generate the actual `printer.cfg` with proper `[include]` directives

Avoid generic names like `printer.cfg` in the repository. The actual `printer.cfg` on the Raspberry Pi is generated by the install script.
...
./klipper_setup/klipper_config/install_x_axis_stepper_cfg.sh  # For X-axis config
# OR
./klipper_setup/klipper_config/install_printer_cfg.sh         # For full printer config
